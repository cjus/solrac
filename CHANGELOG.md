...
## v0.10.1 — web mic respects secure-context requirement + Tailscale recipe

Patch release on top of v0.10.0. Fixes the web UI's mic button silently throwing `Cannot read properties of undefined (reading 'getUserMedia')` when the UI is opened over plain HTTP from a non-localhost origin (LAN IP, Tailscale 100.x address, etc). The browser's `navigator.mediaDevices` is undefined in non-secure contexts — only HTTPS or `http://localhost` / `http://127.0.0.1` qualify. Pre-fix, the client only checked the server's `VOICE_ENABLED` flag; post-fix, it also feature-detects the browser API and hides the mic button when missing. Speak buttons keep working — `<audio>` doesn't need a secure context.

- **Client feature-detect for `getUserMedia`.** New `hasMicCaptureSupport()` helper in `public/app.js` checks for `navigator.mediaDevices.getUserMedia` and `MediaRecorder`. A new `micCaptureAvailable` flag (distinct from `voiceFeatureAvailable`) gates the mic button alone — speak buttons still ride on the server-side voice flag. When voice is enabled server-side but the browser lacks the mic API, the button hides itself and a one-shot toast reads `mic disabled — needs HTTPS or http://localhost. Speak buttons still work.` The toast is gated by a `micUnavailableToastShown` flag so subsequent `refreshVoiceState` calls (e.g. after `/voice on|off`) don't re-surface it.
- **Docs: Tailscale serve as a first-class deployment path.** Single-tenant Solrac on a personal machine plus phone / second laptop access wants TLS termination — `tailscale serve` is the cleanest path (auto-provisioned Let's Encrypt cert on the MagicDNS hostname, tailnet-scoped). The recipe lives in four places: `docs/SETUP.md §13` (first-time enablement; replaces the vague "consider Tailscale" line), `docs/USAGE.md §Web UI` (new "Reaching the UI from other devices" subsection), `docs/USAGE.md §Voice surface` (mic-specific context with three secure-context fixes), and `docs/RUNBOOK.md` (new "Web UI mic button hidden / `getUserMedia` undefined" section with diagnosis + recovery). All four cross-link. Recipe is explicit about `tailscale funnel` NOT being a substitute — that exposes Solrac to the public internet, defeating the single-tenant posture.
- **No new env vars, no DB change, no SDK pin bump.** Pure client-side fix plus doc additions.
- **Anti-goals.** None reversed. Tailscale is one of three documented options (localhost / Tailscale / reverse proxy with TLS) — recommended for single-tenant, never required.

## v0.10.0 — voice (ElevenLabs STT + TTS) on Telegram and web

Adds optional **voice** on both transports: Telegram voice notes get transcribed via ElevenLabs Scribe and routed through the existing engine path; with `/voice on` (per-chat sticky), Telegram replies attach a TTS voice note. The web UI gets a mic button (composer pre-fill) and a per-message 🔊 button with cached blob replay. Off by default; enabling requires `VOICE_ENABLED=true` plus `ELEVENLABS_API_KEY` + `ELEVENLABS_VOICE_ID`. Voice spend rides an **independent** cost-cap axis — the Anthropic burn cap is unaffected. No SDK, no realtime WebSocket — a typed `fetch` wrapper against two ElevenLabs endpoints.

- **New env vars** (all `VOICE_*` / `ELEVENLABS_*` namespace, scrubbed from the Claude SDK subprocess):
  - `VOICE_ENABLED` — master switch. Required to be `true` for any voice path. Boot rejects `true` without `ELEVENLABS_API_KEY` + `ELEVENLABS_VOICE_ID`.
  - `ELEVENLABS_API_KEY` — required when voice enabled. Needs both Speech-to-Text and Text-to-Speech permissions for full functionality.
  - `ELEVENLABS_VOICE_ID` — required when voice enabled. The TTS voice the agent speaks with.
  - `ELEVENLABS_TTS_MODEL` (default `eleven_flash_v2_5`), `ELEVENLABS_STT_MODEL` (default `scribe_v1`).
  - `ELEVENLABS_TTS_OUTPUT_FORMAT_WEB` (default `mp3_44100_128`), `ELEVENLABS_TTS_OUTPUT_FORMAT_TG` (default `opus_48000_64`) — per-transport audio format.
  - `VOICE_TTS_MAX_CHARS` (default 1500) — TTS length wall; replies above the threshold are sent as text only.
  - `VOICE_REPLY_WORDS_HINT` (default 60, clamped to `[30, 200]` with a warn line) — soft hint passed to the model via a `<voice-mode>` system block.
  - `VOICE_STT_MAX_BYTES` (default 25 MB), `VOICE_STT_MAX_SECONDS` (default 600) — STT input walls; over-cap inputs write `too_long` rows without firing ElevenLabs.
  - `VOICE_HOURLY_COST_CAP_USD` (default $0.25/hr), `VOICE_GLOBAL_HOURLY_COST_CAP_USD` (default $1.00/hr) — independent of the Anthropic caps; sliding 60-min over `voice_events.cost_usd_estimate`.
  - `ELEVENLABS_TTS_PRICE_USD_PER_1K_CHARS` (default `0.05`), `ELEVENLABS_STT_PRICE_USD_PER_HOUR` (default `0.40`) — operator-set price constants for the local estimate. Pin to your plan; the cap fires at the operator-set rate, not ElevenLabs' billing.
- **New SQLite table: `voice_events`.** Parallel to `audit`, not nested. One row per ElevenLabs attempt — `kind` (`stt`|`tts`), `source` (`web`|`telegram`), `status` (`ok`|`denied_cap`|`denied_gate`|`error`), `cost_usd_estimate`, `duration_ms` (STT) or `chars` (TTS), `audit_id` (informational, no FK). Two indexes (`idx_voice_events_chat_ts`, `idx_voice_events_ts`) back the cap queries. Denied-gate STTs that never reach `audit` still get a row here, so the table answers "did we attempt this?" not just "did it succeed?"
- **Independent cost-cap axis.** `db.voiceCostUsedLast60min` + `db.voiceCostUsedGlobalLast60min` mirror the Anthropic cap shape (global → per-chat order, sliding 60-min). Cap-hit writes a `denied_cap` row with `error_message ∈ {chat_voice_cap, global_voice_cap}` and refuses the ElevenLabs call. Anthropic burn (`audit.cost_usd`) and voice burn (`voice_events.cost_usd_estimate`) never sum into the same window.
- **STT path.** Telegram voice notes (`msg.voice`) dispatch through `voice.ts::handleTelegramVoiceStt` → ElevenLabs Scribe → synthesized text Update with the same `update_id` / `chat` / `from` → re-enqueue. The downstream agent loop is identical to a typed message; the audit row stores the transcribed prompt. Web UI mic button → multipart upload to `/api/stt` → `voice.ts::handleWebStt` → composer pre-fill.
- **TTS path.** `/voice on` (alias `/voice 1` / `/voice true`) sets `sessions.voice_replies = 1`; `/voice off` clears it. When enabled, the agent + engine post-turn hooks call `voice.ts::maybeReplyWithVoice` with the final markdown text. Footer (`*✅ engine · tools · time · cost*`) is stripped via regex before tokenizing; markdown walks via `marked` (code → "[code block omitted]", tables → "[table omitted]", lists comma-joined, links unwrapped) so the model speaks prose, not punctuation. Telegram gets a voice-note attachment (`sendVoice`); the web UI fetches `/api/tts` on click and caches the blob URL on the audio element so replays don't re-bill.
- **Allowlist + safety reuse.** Voice goes through the same `policy.ts::gateUpdate` allowlist gate as text — group chats still gate on `from.id`, not `chat.id`. The three-tier permission policy is unchanged; voice doesn't bypass any existing safety machinery. The `<voice-mode>` system block telling the model to keep replies under N words is appended to the SOLRAC.md envelope, never replacing it.
- **Subprocess env scrub.** `agent.ts::sanitizedSubprocessEnv` adds `ELEVENLABS_*` and `VOICE_*` prefix exclusions alongside `TELEGRAM_*` / `TG_*` / `REMOTE_*` / `LOCAL_*`. A compromised model can't `Bash(echo $ELEVENLABS_API_KEY)` to exfiltrate the billed credential.
- **New slash command: `/voice on|off`** (no-arg queries current state). Both `/voice` and `:voice` dispatch the same handler. Added to `HELP_COMMANDS_MD` and `BOT_COMMAND_REGISTRY`.
- **Web UI.** Voice mode badge in the chat header (click to disable). Mic button in the composer with `.recording` pulse animation. Per-message speak button positioned bottom-right; play/stop toggle (🔊 ↔ ⏹) with cached blob replay. HTML5 audio controls hidden — the speak button is the only control. New `/api/voice/state` endpoint exposes the chat's voice-replies flag so the badge survives reloads. MediaRecorder picks `audio/webm;codecs=opus` on Chromium, `audio/mp4` on Safari.
- **Telegram client surface.** `TelegramClient` gains typed `sendVoice` + `sendAudio` (multipart, `audio/ogg` default for `sendVoice`, `audio/mpeg` for `sendAudio`). Private `callMultipart` + `buildAudioForm` helpers. Web client throws "not implemented" on these — the browser path uses the per-message speak button instead.
- **Footer-strip for TTS.** The agent footer sentinel `*✅ engine · N tools · X.Xs · $X.XXXX*` is stripped from response text before TTS so the model doesn't speak its own audit chip. Implemented as a single regex (`/\*\s*✅[^*\n]*\*/g`) in `voice.ts::stripMarkdownForSpeech` ahead of the markdown walk.
- **No new runtime deps.** ElevenLabs is direct `fetch` against two endpoints; ~175 lines in `src/elevenlabs.ts`. Same minimalism as `src/telegram.ts` — one library on the public dep surface would have been disproportionate.
- **No SDK pin bump.** Claude Agent SDK pin stays at `0.2.119`.
- **Tests.** 29 new pure-logic tests in `src/voice.test.ts` cover `stripMarkdownForSpeech` (code blocks, tables, lists, links, footer regex variants), cost math (`estimateSttCostUsd`, `estimateTtsCostUsd`), and `buildVoiceModePrompt` framing. Full suite passes (828/828 across all files); typecheck clean.
- **Docs.** Voice covered in CONFIG (env reference + boot validation), ARCHITECTURE (transport section, voice_events schema, independent cost-cap axis), USAGE (operator how-to, privacy note), OPERATIONS (5 SQL queries for spend forensics), FEATURES (transport + safety bullets), README (pitch line + comparison clause + punched-up tagline leading with "configure, hack, **converse with**"), SCHEMA (`voice_events` table reference + 6 debugging queries), GLOSSARY (ElevenLabs, STT, TTS, voice cost cap, voice mode, voice_events), RUNBOOK (voice cost runaway + ElevenLabs error recovery, including the shell-overrides-`.env` gotcha).
- **Anti-goals.** No anti-goals reversed. Voice is off by default. ElevenLabs is the third runtime HTTP provider after Anthropic / Telegram / OpenRouter / Ollama / LMStudio, but it's behind a master switch with its own independent cost cap. No new HTTP framework. No Telegram framework runtime. No SDK pin bump. No sub-agents enabled.

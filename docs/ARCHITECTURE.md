...
- **Heavy deps optional or pinned.** Blessed integrations dynamic-import their SDKs and gracefully no-op when absent. `@notionhq/client` is a runtime dep in `package.json`; `googleapis` + `google-auth-library` are devDeps only (operators who want Gmail in production run `npm install --save googleapis google-auth-library`). The dynamic-import pattern lets the integration tolerate a missing or broken `node_modules` rather than crashing boot.
- **Built-in integrations shipped:**
  - `time` — current time + timestamp formatting (zero deps, always available when integrations are enabled). Educational reference for operators learning the pattern.
